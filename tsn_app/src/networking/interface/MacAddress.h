...
class MacAddress : public IAddress {
   ...
   public:
    MacAddress() : m_family(0), m_ifindex(-1), m_protocol(0) {
        memset(this->m_mac, 0x00, 6);
    }

    MacAddress(unsigned char* mac, unsigned char family = 0, unsigned char ifindex = -1, unsigned char protocol = 0) : m_family(family), m_ifindex(ifindex), m_protocol(protocol) {
