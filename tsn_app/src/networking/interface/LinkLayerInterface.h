...
class LinkLayerInterface : public IInterface {
   ...
   public:
    LinkLayerInterface() : m_name(""), m_index(-1) {
    }

    LinkLayerInterface(const char* name) : m_name(name) {
        this->m_index = LinkLayerInterface::getIndex(name);
        this->m_macAddress.reset(LinkLayerInterface::getMacAddress(name));
    }

    LinkLayerInterface(const char* name, std::shared_ptr<MacAddress> macAddress) : m_name(name), m_macAddress(macAddress) {
